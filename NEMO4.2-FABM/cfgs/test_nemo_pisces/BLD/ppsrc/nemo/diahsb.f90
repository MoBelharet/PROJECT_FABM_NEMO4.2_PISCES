...
CONTAINS
   ...
   SUBROUTINE dia_hsb( kt, Kbb, Kmm )
      ...
      DO jk = 1, jpkm1           ! volume
         ztmpk(:,:,jk,1) =   surf    (:,:) * e3t_0(:,:,jk)*tmask(:,:,jk)   &
            &              - surf_ini(:,:) * e3t_ini(:,:,jk    )*tmask_ini(:,:,jk)
      END DO
      DO jk = 1, jpkm1           ! heat
         ztmpk(:,:,jk,2) = ( surf    (:,:) * e3t_0(:,:,jk)*ts(:,:,jk,jp_tem,Kmm)   &
            &              - surf_ini(:,:) *         hc_loc_ini(:,:,jk) )
      END DO
      DO jk = 1, jpkm1           ! salt
         ztmpk(:,:,jk,3) = ( surf    (:,:) * e3t_0(:,:,jk)*ts(:,:,jk,jp_sal,Kmm)   &
            &              - surf_ini(:,:) *         sc_loc_ini(:,:,jk) )
      END DO
      DO jk = 1, jpkm1           ! total ocean volume
         ztmpk(:,:,jk,4) = surf(:,:) * e3t_0(:,:,jk) * tmask(:,:,jk)
      END DO
         ...
         ELSE
            ...
            DO jk = 1, jpk
              ! if ice sheet/oceqn coupling, need to mask ini variables here (mask could change at the next NEMO instance).
               e3t_ini   (:,:,jk) = e3t_0(:,:,jk)                      * tmask(:,:,jk)  ! initial vertical scale factors
               tmask_ini (:,:,jk) = tmask(:,:,jk)                                       ! initial mask
               hc_loc_ini(:,:,jk) = ts(:,:,jk,jp_tem,Kmm) * e3t_0(:,:,jk) * tmask(:,:,jk)  ! initial heat content
               sc_loc_ini(:,:,jk) = ts(:,:,jk,jp_sal,Kmm) * e3t_0(:,:,jk) * tmask(:,:,jk)  ! initial salt content
            END DO
