   ...
   SUBROUTINE fld_bdy_interp(pdta_read, pdta_read_z, pdta_read_dz, pdta, kgrd, kbdy, pfv, ldtotvel, Kmm )
      ...
      DO jb = 1, ipi
         ...
         CASE(2)            ! depth of U points: we must not use gdept_n as we don't want to do a communication
            ...
            zdhalf(1) = 0.0_wp
            zdepth(1) = 0.5_wp * e3uw_0(ji,jj,1)
            DO jk = 2, jpk                               ! vertical sum
               ...
               zdhalf(jk) = zdhalf(jk-1) + e3u_0(ji,jj,jk-1)
               zdepth(jk) =          zcoef  * ( zdhalf(jk  ) + 0.5_wp * e3uw_0(ji,jj,jk))  &
                  &         + (1._wp-zcoef) * ( zdepth(jk-1) +          e3uw_0(ji,jj,jk))
            END DO
         CASE(3)            ! depth of V points: we must not use gdept_n as we don't want to do a communication
            !                 --> copy what is done for gdept_n in domvvl...
            zdhalf(1) = 0.0_wp
            zdepth(1) = 0.5_wp * e3vw_0(ji,jj,1)
            DO jk = 2, jpk                               ! vertical sum
               ...
               zdhalf(jk) = zdhalf(jk-1) + e3v_0(ji,jj,jk-1)
               zdepth(jk) =          zcoef  * ( zdhalf(jk  ) + 0.5_wp * e3vw_0(ji,jj,jk))  &
                     + (1._wp-zcoef) * ( zdepth(jk-1) +          e3vw_0(ji,jj,jk))
            END DO
         END SELECT
         !         
      ...
      CASE(2)                                 ! adjust the U-transport term
         DO jb = 1, ipi
            ...
            DO jk = 1, jpk                                ! calculate transport on model grid
               ztrans_new = ztrans_new +      pdta(jb,1,jk ) * e3u_0(ji,jj,jk) * umask(ji,jj,jk)
            ENDDO
            DO jk = 1, jpk                                ! make transport correction
               IF(ldtotvel) THEN ! bdy data are total velocity so adjust bt transport term to match input data
                  pdta(jb,1,jk) = ( pdta(jb,1,jk) + ( ztrans - ztrans_new ) * r1_hu_0(ji,jj) ) * umask(ji,jj,jk)
               ELSE              ! we're just dealing with bc velocity so bt transport term should sum to zero
                  pdta(jb,1,jk) =   pdta(jb,1,jk) + (  0._wp - ztrans_new ) * r1_hu_0(ji,jj)   * umask(ji,jj,jk)
               ENDIF
            ENDDO
         ENDDO
      ...
      CASE(3)                                 ! adjust the V-transport term
         DO jb = 1, ipi
            ...
            ztrans_new = 0._wp
            DO jk = 1, jpk                                ! calculate transport on model grid
               ztrans_new = ztrans_new +      pdta(jb,1,jk ) * e3v_0(ji,jj,jk) * vmask(ji,jj,jk)
            ENDDO
            DO jk = 1, jpk                                ! make transport correction
               IF(ldtotvel) THEN ! bdy data are total velocity so adjust bt transport term to match input data
                  pdta(jb,1,jk) = ( pdta(jb,1,jk) + ( ztrans - ztrans_new ) * r1_hv_0(ji,jj) ) * vmask(ji,jj,jk)
               ELSE              ! we're just dealing with bc velocity so bt transport term should sum to zero
                  pdta(jb,1,jk) =   pdta(jb,1,jk) + (  0._wp - ztrans_new ) * r1_hv_0(ji,jj)   * vmask(ji,jj,jk)
               ENDIF
            ENDDO
         ENDDO
      END SELECT
