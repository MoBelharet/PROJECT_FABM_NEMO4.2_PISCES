   ...
   SUBROUTINE sbc_rnf_div( phdivn, Kmm )
         ...
         ELSE                    !* variable volume case
            DO jj = ntsj-(  nn_hls-( nn_hls+ nn_hls )*nthb), ntej+(  nn_hls -( nn_hls + nn_hls)*ntht) ; DO ji = ntsi-( nn_hls-( nn_hls+ nn_hls)*nthl), ntei+(  nn_hls-( nn_hls+ nn_hls)*nthr)         ! update the depth over which runoffs are distributed
               ...
               DO jk = 1, nk_rnf(ji,jj)                             ! recalculates h_rnf to be the depth in metres
                  h_rnf(ji,jj) = h_rnf(ji,jj) + e3t_0(ji,jj,jk)   ! to the bottom of the relevant grid box
               END DO
            END DO   ;   END DO
      ...
      ELSE                       !==   runoff put only at the surface   ==!
         DO jj = ntsj-(  nn_hls-( nn_hls+ nn_hls )*nthb), ntej+(  nn_hls -( nn_hls + nn_hls)*ntht) ; DO ji = ntsi-( nn_hls-( nn_hls+ nn_hls)*nthl), ntei+(  nn_hls-( nn_hls+ nn_hls)*nthr)
            h_rnf (ji,jj)   = e3t_0(ji,jj,1)        ! update h_rnf to be depth of top box
         END DO   ;   END DO
         DO jj = ntsj-(  nn_hls-1-( nn_hls-1+ nn_hls )*nthb), ntej+(  nn_hls -( nn_hls + nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+ nn_hls)*nthl), ntei+(  nn_hls-( nn_hls+ nn_hls-1)*nthr)
            phdivn(ji,jj,1) = phdivn(ji,jj,1) - ( rnf(ji,jj) + rnf_b(ji,jj) ) * zfact * r1_rho0 / e3t_0(ji,jj,1)
         END DO   ;   END DO
      ENDIF
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_rnf in configuration namelist' )
      ...
      IF( ln_rnf_depth ) THEN                    ! depth of runoffs set from a file
         ...
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)                           ! set the associated depth
            h_rnf(ji,jj) = 0._wp
            DO jk = 1, nk_rnf(ji,jj)
               h_rnf(ji,jj) = h_rnf(ji,jj) + e3t_0(ji,jj,jk)
            END DO
         END DO   ;   END DO
         !
