   ...
   SUBROUTINE zdf_drg( kt, Kmm, k_mk, pCdmin, pCdmax, pz0, pke0, pCd0,   &   ! <<== in 
      ...
      IF( l_log_not_linssh ) THEN     !==  "log layer"  ==!   compute Cd and -Cd*|U|
         DO jj = ntsj-(  nn_hls-1-( nn_hls-1+ nn_hls-1 )*nthb), ntej+(  nn_hls-1 -( nn_hls-1 + nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+ nn_hls-1)*nthl), ntei+(  nn_hls-1-( nn_hls-1+ nn_hls-1)*nthr)
            ...
            zzz = 0.5_wp * e3t_0(ji,jj,imk)           ! altitude below/above (top/bottom) the boundary
            !
...
!!gm bug : time step is only rn_Dt (not 2 rn_Dt if euler start !)
      ...
      DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
         ...
         zCdu = 0.5*( rCdU_bot(ji+1,jj)+rCdU_bot(ji,jj) ) / e3u_0(ji,jj,ikbu)
         zCdv = 0.5*( rCdU_bot(ji,jj+1)+rCdU_bot(ji,jj) ) / e3v_0(ji,jj,ikbv)
         !
      ...
      IF( ln_isfcav ) THEN        ! ocean cavities
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
            ...
            !
            ! Apply stability criteria on absolute value  : abs(bfr/e3) < 1/(2dt) => bfr/e3 > -1/(2dt)
            zCdu = 0.5*( rCdU_top(ji+1,jj)+rCdU_top(ji,jj) ) / e3u_0(ji,jj,ikbu)    ! NB: Cdtop masked
            zCdv = 0.5*( rCdU_top(ji,jj+1)+rCdU_top(ji,jj) ) / e3v_0(ji,jj,ikbv)
            !
            pua(ji,jj,ikbu) = pua(ji,jj,ikbu) + MAX(  zCdu , zm1_2dt  ) * pub(ji,jj,ikbu)
            pva(ji,jj,ikbv) = pva(ji,jj,ikbv) + MAX(  zCdv , zm1_2dt  ) * pvb(ji,jj,ikbv)
         END DO   ;   END DO
