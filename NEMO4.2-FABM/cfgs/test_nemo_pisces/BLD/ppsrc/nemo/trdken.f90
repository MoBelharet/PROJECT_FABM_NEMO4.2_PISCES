...
CONTAINS
   ...
   SUBROUTINE trd_ken( putrd, pvtrd, ktrd, kt, Kmm )
      ...
      nkstp = kt
      DO jk = 1, jpkm1
         bu   (:,:,jk) =    e1e2u(:,:) * e3u_0(:,:,jk)
         bv   (:,:,jk) =    e1e2v(:,:) * e3v_0(:,:,jk)
         r1_bt(:,:,jk) = r1_e1e2t(:,:) / e3t_0(:,:,jk) * tmask(:,:,jk)
      END DO
      !
      zke(:,:,jpk) = 0._wp
      zke(1:nn_hls,:, : ) = 0._wp
...
!                  ikbu = mbku(ji,jj)          ! deepest ocean u- & v-levels
!                  ikbv = mbkv(ji,jj)
!                  z2dx(ji,jj) = uu(ji,jj,ikbu,Kmm) * bfrua(ji,jj) * uu(ji,jj,ikbu,Kmm) / e3u_0(ji,jj,ikbu)
!                  z2dy(ji,jj) = uu(ji,jj,ikbu,Kmm) * bfrva(ji,jj) * vv(ji,jj,ikbv,Kmm) / e3v_0(ji,jj,ikbv)
!               END DO
!            END DO
!            zke2d(1,:) = 0._wp   ;   zke2d(:,1) = 0._wp
!            DO jj = 2, jpj
...
!         ENDIF
   ...
   SUBROUTINE ken_p2k( kt , pconv, Kmm )
      ...
      zconv(:,:,1) = zcoef * ( 2._wp * rhd(:,:,1) ) * ww(:,:,1) * e3w_0(:,:,1)

      ! interior value (2=<jk=<jpkm1)
      DO jk = 2, jpk
         zconv(:,:,jk) = zcoef * ( rhd(:,:,jk) + rhd(:,:,jk-1) ) * ww(:,:,jk) * e3w_0(:,:,jk)
      END DO
