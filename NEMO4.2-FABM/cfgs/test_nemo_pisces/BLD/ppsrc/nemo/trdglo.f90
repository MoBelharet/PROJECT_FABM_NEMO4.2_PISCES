         ...
         !
      	...
      	CASE( 'TRA' )          !==  Tracers (T & S)  ==!
      	   DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! global sum of mask volume trend and trend*T (including interior mask)
	            zvm = e1e2t(ji,jj) * e3t_0(ji,jj,jk) * tmask(ji,jj,jk) * tmask_i(ji,jj)
...
!!gm end
   ...
   SUBROUTINE glo_dyn_wri( kt, Kmm )
      ...
      IF( MOD( kt, nn_trd ) == 0 .OR. kt == nit000 .OR. kt == nitend ) THEN
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  0) ; DO ji = ntsi-( 1), ntei+(  0)
            zkx(ji,jj,jk) = zcof * e2u(ji,jj) * e3u_0(ji,jj,jk)   &
               &                              *  uu(ji,jj,jk,Kmm) * ( rhop(ji,jj,jk) + rhop(ji+1,jj,jk) )
            zky(ji,jj,jk) = zcof * e1v(ji,jj) * e3v_0(ji,jj,jk)   &
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)    ! Density flux divergence at t-point
            zkepe(ji,jj,jk) = - (  zkz(ji,jj,jk) - zkz(ji  ,jj  ,jk+1)               &
               ...
               &              / ( e1e2t(ji,jj) * e3t_0(ji,jj,jk) ) * tmask(ji,jj,jk) * tmask_i(ji,jj)
         END DO   ;   END DO   ;   END DO
         ...
         peke = 0._wp
         DO jk = 1, jpkm1
            peke = peke + SUM( zkepe(:,:,jk) * gdept_0(:,:,jk) * e1e2t(:,:)   &
               &                               * e3t_0(:,:,jk) )
         END DO
         peke = grav * peke
...
9449     FORMAT(' 0 > static instability mixing     * t  ',e20.13,'       ',e20.13)
   ...
   SUBROUTINE trd_glo_init( Kmm )
      ...
      tvolt = 0._wp
      DO jk = 1, jpkm1
         tvolt = tvolt + SUM( e1e2t(:,:) * e3t_0(:,:,jk) * tmask(:,:,jk) * tmask_i(:,:) )
      END DO
...
!!gm :  bug?  je suis quasi sur que le produit des tmask_i ne correspond pas exactement au umask_i et vmask_i !
      ...
      DO jk =  1,  jpk  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
         tvolu = tvolu + e1u(ji,jj) * e2u(ji,jj) * e3u_0(ji,jj,jk)   &
            &                                       * tmask_i(ji+1,jj  ) * tmask_i(ji,jj) * umask(ji,jj,jk)
         tvolv = tvolv + e1v(ji,jj) * e2v(ji,jj) * e3v_0(ji,jj,jk)   &
