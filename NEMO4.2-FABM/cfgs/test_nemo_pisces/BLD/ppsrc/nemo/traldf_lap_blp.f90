...
CONTAINS
   ...
   SUBROUTINE tra_ldf_lap_t( kt, Kmm, kit000, cdtype, pahu, pahv, ktah,                   &
      ...
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij), ntej+(  iij-1) ; DO ji = ntsi-( iij), ntei+(  iij-1)            !== First derivative (gradient)  ==!
         zaheeu(ji,jj,jk) = zsign * pahu(ji,jj,jk) * e2_e1u(ji,jj) * e3u_0(ji,jj,jk)   !!gm   * umask(ji,jj,jk) pah masked!
         zaheev(ji,jj,jk) = zsign * pahv(ji,jj,jk) * e1_e2v(ji,jj) * e3v_0(ji,jj,jk)   !!gm   * vmask(ji,jj,jk)
