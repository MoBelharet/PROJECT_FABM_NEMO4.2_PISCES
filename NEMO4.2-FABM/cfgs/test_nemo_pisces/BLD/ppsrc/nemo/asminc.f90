...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_asminc in configuration namelist' )
         ...
         DO jt = 1, nn_divdmp
            ...
            DO jk = 1, jpkm1           ! zhdiv = e1e1 * div
               ...
               DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
                  zhdiv(ji,jj) = (  e2u(ji  ,jj) * e3u_0(ji  ,jj,jk) * u_bkginc(ji  ,jj,jk)    &
                     &            - e2u(ji-1,jj) * e3u_0(ji-1,jj,jk) * u_bkginc(ji-1,jj,jk)    &
                     &            + e1v(ji,jj  ) * e3v_0(ji,jj  ,jk) * v_bkginc(ji,jj  ,jk)    &
                     &            - e1v(ji,jj-1) * e3v_0(ji,jj-1,jk) * v_bkginc(ji,jj-1,jk)  ) &
                     &            / e3t_0(ji,jj,jk)
               END DO   ;   END DO
   ...
   SUBROUTINE tra_asm_inc( kt, Kbb, Kmm, pts, Krhs )
      ...
      IF( ln_temnofreeze ) THEN
         DO jk = 1, jpkm1
           CALL eos_fzp( pts(:,:,jk,jp_sal,Kmm), fzptnz(:,:,jk), gdept_0(:,:,jk) )
         END DO
      ENDIF
         !
         !                             !--------------------------------------
   ...
   SUBROUTINE ssh_asm_inc( kt, Kbb, Kmm )
      ...
      ELSEIF ( ln_asmdin ) THEN     ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            e3t_0(:,:,:) = e3t_0(:,:,:)
!!gm why not e3u_0(:,:,:), e3v_0(:,:,:), gdept_0(:,:,:) ????
            !
            DEALLOCATE( ssh_bkg    )
...
!                 DO jk = jpk-1, 2, -1
!                   IF ((mld > gdepw_0(ji,jj,jk)) .and. (mld < gdepw_0(ji,jj,jk+1))) THEN
!                     mld=gdepw_0(ji,jj,jk+1)
!                     jkmax=jk
!                   ENDIF
!                 ENDDO
!
