...
!
!                     !compute weight
!                     zdzp1 = MAX(0._wp,pdepw_b(ji,jj,jk+1) - gdepw_0(ji,jj,jk+1))
!                     zdzm1 = MAX(0._wp,gdepw_0(ji,jj,jk  ) - pdepw_b(ji,jj,jk  ))
!                     zdz   = e3t_0(ji,jj,jk) - zdzp1 - zdzm1 ! if isf : e3t = gdepw_0(ji,jj,jk+1)- gdepw_0(ji,jj,jk)
!
...
!                        &                   + zdzm1*ts(ji,jj,jk-1,jp_tem,Kbb) )/e3t_0(ji,jj,jk)
!
!                     ts(ji,jj,jk,jp_sal,Kmm) = ( zdzp1*ts(ji,jj,jk+1,jp_sal,Kbb) &
!                        &                   + zdz  *ts(ji,jj,jk  ,jp_sal,Kbb) &
!                        &                   + zdzm1*ts(ji,jj,jk-1,jp_sal,Kbb) )/e3t_0(ji,jj,jk)
!
!                  END IF
!               END DO
!            END DO
...
!      END IF
   ...
   SUBROUTINE isfcpl_vol(Kmm)
      ...
      DO jk = 1, jpk                                 ! Horizontal slab
         ...
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
            zqvoln(ji,jj,jk) =   &
               &  (   e2u(ji  ,jj  ) * e3u_0(ji  ,jj  ,jk) * uu(ji  ,jj  ,jk,Kmm)    &
      ...
      DO jk = 1,jpk-1
         DO jj = Njs0,Nje0
            DO ji = Nis0,Nie0
               ...
               ! volume diff
               zdvol =   e3t_0(ji,jj,jk) *  tmask  (ji,jj,jk)   &
                  &   - ze3t_b(ji,jj,jk    ) * ztmask_b(ji,jj,jk)

               ! heat diff
               zdtem = ts (ji,jj,jk,jp_tem,Kmm) *  e3t_0(ji,jj,jk) *  tmask  (ji,jj,jk)   &
                     - zt_b(ji,jj,jk)        * ze3t_b(ji,jj,jk) * ztmask_b(ji,jj,jk)

               ! salt diff
               zdsal = ts(ji,jj,jk,jp_sal,Kmm) *  e3t_0(ji,jj,jk) *  tmask  (ji,jj,jk)   &
