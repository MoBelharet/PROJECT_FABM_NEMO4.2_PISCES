   ...
   SUBROUTINE p4z_sed( kt, knt, Kbb, Kmm, Krhs )
      ...
      IF( .NOT.lk_sed ) THEN
         ...
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
           IF( tmask(ji,jj,1) == 1 ) THEN
              ...
              zdep  = LOG10( gdepw_0(ji,jj,ikt+1) )
              ...
              zbureff(ji,jj) = 0.013 + 0.53 * zflx**2 / ( 7.0 + zflx )**2 * MIN(gdepw_0(ji,jj,ikt+1) / 1000.00, 1.0)
           ENDIF
         END DO   ;   END DO
         !
      ENDIF
      ...
      IF( .NOT.lk_sed ) THEN
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
            ...
            zsedcal(ji,jj) = (1.0 - zrivalk) * zcaloss * e3t_0(ji,jj,ikt) 
            zsedsi (ji,jj) = (1.0 - zrivsil) * zsiloss * e3t_0(ji,jj,ikt) 
         END DO   ;   END DO
      ENDIF
      !
      DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
      ...
      IF( .NOT.lk_sed ) THEN
         ...
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
            ...
            sdenit(ji,jj) = rdenit * zpdenit * e3t_0(ji,jj,ikt)
            zsedc(ji,jj)   = (1. - zrivno3) * zwstpoc * e3t_0(ji,jj,ikt)
            IF( ln_p5z ) THEN
