...
CONTAINS
   ...
   SUBROUTINE crs_fld( kt, Kmm )
      ...
      IF( iom_use( "ke") ) THEN     !      kinetic energy
         z3d(:,:,jk) = 0._wp 
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
            zztmp  = r1_e1e2t(ji,jj) / e3t_0(ji,jj,jk)
            z3d(ji,jj,jk) = 0.25_wp * zztmp * (                                    &
               &            uu(ji-1,jj,jk,Kmm)**2 * e2u(ji-1,jj) * e3u_0(ji-1,jj,jk)   &
               &          + uu(ji  ,jj,jk,Kmm)**2 * e2u(ji  ,jj) * e3u_0(ji  ,jj,jk)   &
               &          + vv(ji,jj-1,jk,Kmm)**2 * e1v(ji,jj-1) * e3v_0(ji,jj-1,jk)   &
               &          + vv(ji,jj  ,jk,Kmm)**2 * e1v(ji,jj  ) * e3v_0(ji,jj  ,jk)   )
         END DO   ;   END DO   ;   END DO
