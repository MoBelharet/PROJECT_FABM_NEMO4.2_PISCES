...
!!gm
      ...
      IF( l_trdtra )   THEN                      ! save the vertical diffusive trends for further diagnostics
         DO jk = 1, jpk
            ztrdt(:,:,jk) = (   (  pts(:,:,jk,jp_tem,Kaa)*e3t_0(:,:,jk)     &
               &                 - pts(:,:,jk,jp_tem,Kbb)*e3t_0(:,:,jk)  )  &
               &              / (  e3t_0(:,:,jk)*rDt  )   )                 &
               &          - ztrdt(:,:,jk)
            ztrds(:,:,jk) = (   (  pts(:,:,jk,jp_sal,Kaa)*e3t_0(:,:,jk)     &
               &                 - pts(:,:,jk,jp_sal,Kbb)*e3t_0(:,:,jk)  )  &
               &             / (   e3t_0(:,:,jk)*rDt  )   )                 &
               &          - ztrds(:,:,jk)
         END DO
   ...
   SUBROUTINE tra_zdf_imp( kt, kit000, cdtype, p2dt, Kbb, Kmm, Krhs, pt, Kaa, kjpt )
      ...
      DO jn = 1, kjpt                                 !  tracer loop  !
         ...
         IF(  ( cdtype == 'TRA' .AND. ( jn == jp_tem .OR. ( jn == jp_sal .AND. ln_zdfddm ) ) ) .OR.   &
            ...
            IF( ln_zad_Aimp ) THEN         ! Adaptive implicit vertical advection
               DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
                  zzwi = - p2dt * zwt(ji,jj,jk  ) / e3w_0(ji,jj,jk  )
                  zzws = - p2dt * zwt(ji,jj,jk+1) / e3w_0(ji,jj,jk+1)
                  zwd(ji,jj,jk) = e3t_0(ji,jj,jk) - zzwi - zzws   &
            ...
            ELSE
               DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
                  zwi(ji,jj,jk) = - p2dt * zwt(ji,jj,jk  ) / e3w_0(ji,jj,jk)
                  zws(ji,jj,jk) = - p2dt * zwt(ji,jj,jk+1) / e3w_0(ji,jj,jk+1)
                  zwd(ji,jj,jk) = e3t_0(ji,jj,jk) - zwi(ji,jj,jk) - zws(ji,jj,jk)
               END DO   ;   END DO   ;   END DO
            ENDIF
            !
         ...
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)         !* 2nd recurrence:    Zk = Yk - Ik / Tk-1  Zk-1
            pt(ji,jj,1,jn,Kaa) =        e3t_0(ji,jj,1) * pt(ji,jj,1,jn,Kbb)    &
               &               + p2dt * e3t_0(ji,jj,1) * pt(ji,jj,1,jn,Krhs)
         END DO   ;   END DO
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
            zrhs =        e3t_0(ji,jj,jk) * pt(ji,jj,jk,jn,Kbb)    &
               & + p2dt * e3t_0(ji,jj,jk) * pt(ji,jj,jk,jn,Krhs)   ! zrhs=right hand side
