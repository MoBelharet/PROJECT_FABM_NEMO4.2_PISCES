...
CONTAINS
      ...
      IF( l_hth ) THEN
         ...
         IF( iom_use( 'mlddzt' ) .OR. iom_use( 'mld_dt02' ) .OR. iom_use( 'topthdep' )   &
            ...
            DO jj = ntsj-( 1), ntej+( 1 ) ; DO ji = ntsi-( 1), ntei+( 1)
               zztmp = gdepw_0(ji,jj,mbkt(ji,jj)+1) 
               hth     (ji,jj) = zztmp
               zabs2   (ji,jj) = zztmp
               ztm2    (ji,jj) = zztmp
         ...
         IF(  iom_use( 'mld_dt02' ) .OR. iom_use( 'topthdep' ) .OR. iom_use( 'mldr10_3' ) .OR.  &    
            ...
            DO jk =  jpkm1,  nlb10,  -1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! loop from bottom to nlb10
               !
               zzdep = gdepw_0(ji,jj,jk) * tmask(ji,jj,1)
               !
   ...
   SUBROUTINE dia_hth_dep( Kmm, ptem, pdept )
      ...
      DO jj = ntsj-( 1), ntej+( 1 ) ; DO ji = ntsi-( 1), ntei+( 1)
         !
         zzdep = gdepw_0(ji,jj,mbkt(ji,jj)+1)       ! depth of the ocean bottom
         !
         iid = iktem(ji,jj)
         IF( iid /= 1 ) THEN 
             zztmp =     gdept_0(ji,jj,iid  )   &                     ! linear interpolation
               &  + (    gdept_0(ji,jj,iid+1) - gdept_0(ji,jj,iid)                            )   &
   ...
   SUBROUTINE dia_hth_htc( Kmm, pdep, pt, phtc )
      ...
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
         IF( ( gdepw_0(ji,jj,jk+1) < pdep ) .AND. ( tmask(ji,jj,jk) == 1 ) ) THEN
             ilevel(ji,jj) = jk+1
             zthick(ji,jj) = zthick(ji,jj) + e3t_0(ji,jj,jk)
             phtc  (ji,jj) = phtc  (ji,jj) + e3t_0(ji,jj,jk) * pt(ji,jj,jk)
         ENDIF
      END DO   ;   END DO   ;   END DO
      !
      DO jj = ntsj-( 1), ntej+( 1 ) ; DO ji = ntsi-( 1), ntei+( 1)
         ...
         IF( tmask(ji,jj,ik) == 1 ) THEN
            zthick(ji,jj) = MIN ( gdepw_0(ji,jj,ik+1), pdep ) - zthick(ji,jj)   ! remaining thickness to reach dephw pdep
