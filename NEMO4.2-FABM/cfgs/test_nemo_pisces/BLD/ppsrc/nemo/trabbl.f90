      ...
      DO jn = 1, kjpt                                            ! tracer loop
         ...
         DO jj = ntsj-(  1-( 1+ 0 )*nthb), ntej+(  0 -( 0 + 1)*ntht) ; DO ji = ntsi-( 1-( 1+ 0)*nthl), ntei+(  0-( 0+ 1)*nthr)            ! CAUTION start from i=1 to update i=2 when cyclic east-west
            IF( utr_bbl(ji,jj) /= 0.e0 ) THEN            ! non-zero i-direction bbl advection
               ...
               zbtr = r1_e1e2t(iis,jj) / e3t_0(iis,jj,ikus)
            ...
            IF( vtr_bbl(ji,jj) /= 0.e0 ) THEN            ! non-zero j-direction bbl advection
               ...
               !
               ! up  -slope T-point (shelf bottom point)
               zbtr = r1_e1e2t(ji,ijs) / e3t_0(ji,ijs,ikvs)
               ...
               DO jk = ikvs, ikvd-1                            ! down-slope upper to down T-point (deep column)
                  zbtr = r1_e1e2t(ji,ijd) / e3t_0(ji,ijd,jk)
               ...
               zbtr = r1_e1e2t(ji,ijd) / e3t_0(ji,ijd,ikvd)
