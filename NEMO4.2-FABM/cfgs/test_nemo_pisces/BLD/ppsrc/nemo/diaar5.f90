...
CONTAINS
   ...
   SUBROUTINE dia_ar5( kt, Kmm )
      ...
      IF( iom_use( 'volcello' ) .OR. iom_use( 'masscello' )  ) THEN
         ...
         DO jk = 1, jpkm1
            zrhd(:,:,jk) = e1e2t(:,:) * e3t_0(:,:,jk) * tmask(:,:,jk)
         END DO
         DO jk = 1, jpk
            z3d(:,:,jk) =  rho0 * e3t_0(:,:,jk) * tmask(:,:,jk)
         END DO
      ...
      IF( iom_use( 'botpres' ) .OR. iom_use( 'sshthster' )  .OR. iom_use( 'sshsteric' )  ) THEN
         ...
         DO jk = 1, jpkm1
            zbotpres(:,:) = zbotpres(:,:) + e3t_0(:,:,jk) * zrhd(:,:,jk)
         END DO
         IF( ln_linssh ) THEN
            IF( ln_isfcav ) THEN
...
!!gm
         ...
         zbotpres(:,:) = 0._wp                        ! no atmospheric surface pressure, levitating sea-ice
         DO jk = 1, jpkm1
            zbotpres(:,:) = zbotpres(:,:) + e3t_0(:,:,jk) * rhd(:,:,jk)
         END DO
         IF( ln_linssh ) THEN
            IF ( ln_isfcav ) THEN
               DO ji = 1,jpi
      ...
      IF( iom_use( 'masstot' ) .OR. iom_use( 'temptot' )  .OR. iom_use( 'saltot' )  ) THEN
          ...
          DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
             zztmp = e1e2t(ji,jj) * e3t_0(ji,jj,jk)
      ...
      IF( ln_teos10 ) THEN        ! ! potential temperature (TEOS-10 case)
         IF( iom_use( 'toce_pot') .OR. iom_use( 'temptot_pot' ) .OR. iom_use( 'sst_pot' )  &
            ...
            IF( iom_use( 'temptot_pot' ) ) THEN   ! Output potential temperature in case we use TEOS-10
               z2d(:,:) = 0._wp
               DO jk = 1, jpkm1
                 z2d(:,:) = z2d(:,:) + e1e2t(:,:) * e3t_0(:,:,jk) * ztpot(:,:,jk)
               END DO
             ...
             IF( iom_use( 'tosmint_pot') ) THEN
               z2d(:,:) = 0._wp
               DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
                  z2d(ji,jj) = z2d(ji,jj) + rho0 * e3t_0(ji,jj,jk) *  ztpot(ji,jj,jk)
               END DO   ;   END DO   ;   END DO
         ...
         IF( ln_zdfddm ) THEN
            DO jk =  2,  jpk  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
               IF( rn2(ji,jj,jk) > 0._wp ) THEN
                  zrw = ( gdept_0(ji,jj,jk) - gdepw_0(ji,jj,jk) ) / e3w_0(ji,jj,jk)
                  !
          ...
          ELSE
            DO jk =  1,  jpk  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
               zpe(ji,jj) = zpe(ji,jj) + avt(ji,jj,jk) * MIN(0._wp,rn2(ji,jj,jk)) * rho0 * e3w_0(ji,jj,jk)
            END DO   ;   END DO   ;   END DO
         ENDIF
