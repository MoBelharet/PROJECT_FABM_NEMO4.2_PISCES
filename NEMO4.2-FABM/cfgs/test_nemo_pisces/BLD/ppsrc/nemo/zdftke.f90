...
!
         ...
         !                             !- LHS of Eq.47
         DO jj = ntsj-( nn_hls-1), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls-1)
            zpelc(ji,jj,1) =  MAX( rn2b(ji,jj,1), 0._wp ) * gdepw_0(ji,jj,1) * e3w_0(ji,jj,1)
         END DO   ;   END DO
         DO jk =  2,  jpk  ; DO jj = ntsj-(  nn_hls-1), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls-1), ntei+(  nn_hls-1)
            zpelc(ji,jj,jk)  = zpelc(ji,jj,jk-1) +   &
               &          MAX( rn2b(ji,jj,jk), 0._wp ) * gdepw_0(ji,jj,jk) * e3w_0(ji,jj,jk)
         ...
         !                               ! finite LC depth
         DO jj = ntsj-( nn_hls-1), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls-1)
            zhlc(ji,jj) = gdepw_0(ji,jj,imlc(ji,jj))
         END DO   ;   END DO
         !
         ...
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)                  !* TKE Langmuir circulation source term added to en
            IF ( zus3(ji,jj) /= 0._wp ) THEN
               IF ( gdepw_0(ji,jj,jk) - zhlc(ji,jj) < 0 .AND. wmask(ji,jj,jk) /= 0. ) THEN
                  !                                           ! vertical velocity due to LC
                  zwlc = rn_lc * SIN( rpi * gdepw_0(ji,jj,jk) / zhlc(ji,jj) )
      ...
      IF ( cpl_phioc .and. ln_phioc )  THEN
         ...
         CASE ( 1 ) ! Neumann BC
            DO jj = ntsj-(  nn_hls-1-( nn_hls-1+ nn_hls-1 )*nthb), ntej+(  nn_hls-1 -( nn_hls-1 + nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+ nn_hls-1)*nthl), ntei+(  nn_hls-1-( nn_hls-1+ nn_hls-1)*nthr)
               IF ( phioc(ji,jj) < 0 )  phioc(ji,jj) = 0._wp
               en(ji,jj,2)    = en(ji,jj,2) + ( rn_Dt * phioc(ji,jj) / rho0 ) /e3w_0(ji,jj,2)
               en(ji,jj,1)    = en(ji,jj,2) + (2 * e3t_0(ji,jj,1) * phioc(ji,jj)/rho0) / ( p_avm(ji,jj,1) + p_avm(ji,jj,2) )
...
!!gm       i.e. use gde3w in argument (gdepw_0(:,:,:))
      !
      ...
      IF( nn_etau == 1 ) THEN           !* penetration below the mixed layer (rn_efr fraction)
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
            en(ji,jj,jk) = en(ji,jj,jk) + rn_efr * en(ji,jj,1) * EXP( -gdepw_0(ji,jj,jk) / htau(ji,jj) )   &
      ...
      ELSEIF( nn_etau == 3 ) THEN       !* penetration belox the mixed layer (HF variability)
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
            ...
            en(ji,jj,jk) = en(ji,jj,jk) + zbbrau * zdif * EXP( -gdepw_0(ji,jj,jk) / htau(ji,jj) )   &
               &                        * MAX( 0._wp, 1._wp - zice_fra(ji,jj) ) * wmask(ji,jj,jk) * tmask(ji,jj,1)
         END DO   ;   END DO   ;   END DO
      ENDIF
      !
...
!            zmxlm(ji,jj,1)= vkarmn * MAX ( 1.6 * hsw(ji,jj) , 0.02 )        ! surface mixing length = F(wave height)
      ...
      !
         ...
         !
            ...
            DO jj = ntsj-( nn_hls-1), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls-1)                  ! No sea-ice
 ...
 !!gm Not sure of that coding for ISF....
      ! where wmask = 0 set zmxlm == e3w_0(:,:,:)
      CASE ( 0 )           ! bounded by the distance to surface and bottom
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(  nn_hls-1), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls-1), ntei+(  nn_hls-1)
            zemxl = MIN( gdepw_0(ji,jj,jk) - gdepw_0(ji,jj,mikt(ji,jj)), zmxlm(ji,jj,jk),   &
            &            gdepw_0(ji,jj,mbkt(ji,jj)+1) - gdepw_0(ji,jj,jk) )
            ! wmask prevent zmxlm = 0 if jk = mikt(ji,jj)
            zmxlm(ji,jj,jk) = zemxl * wmask(ji,jj,jk)   &
               &            + MIN( zmxlm(ji,jj,jk) , e3w_0(ji,jj,jk) ) * (1 - wmask(ji,jj,jk))
      ...
      CASE ( 1 )           ! bounded by the vertical scale factor
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(  nn_hls-1), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls-1), ntei+(  nn_hls-1)
            zemxl = MIN( e3w_0(ji,jj,jk), zmxlm(ji,jj,jk) )
      ...
      CASE ( 3 )           ! lup and ldown, |dk[xml]| bounded by e3t :
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(  nn_hls-1), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls-1), ntei+(  nn_hls-1)        ! from the surface to the bottom : lup
            zmxld(ji,jj,jk) =    &
               &    MIN( zmxld(ji,jj,jk-1) + e3t_0(ji,jj,jk-1), zmxlm(ji,jj,jk) )
         END DO   ;   END DO   ;   END DO
