   ...
   SUBROUTINE dyn_ldf_lap_lf_t( kt, Kbb, Kmm, pu, pv, ktuv, pu_rhs, pv_rhs, ktuv_rhs, kpass )
      ...
      CASE ( np_typ_rot )       !==  Vorticity-Divergence operator  ==!
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)                           ! Horizontal slab
            ...
            zcur     = ahmf(ji  ,jj  ,jk) * e3f_0(ji  ,jj  ,jk) * r1_e1e2f(ji  ,jj  )               &   ! ahmf already * by fmask   
               &       * ( e2v(ji+1,jj  ) * pv(ji+1,jj  ,jk) - e2v(ji,jj) * pv(ji,jj,jk)  &
               &         - e1u(ji  ,jj+1) * pu(ji  ,jj+1,jk) + e1u(ji,jj) * pu(ji,jj,jk) )
            zcur_jm1 = ahmf(ji  ,jj-1,jk) * e3f_0(ji  ,jj-1,jk) * r1_e1e2f(ji  ,jj-1)               &   ! ahmf already * by fmask
               &       * ( e2v(ji+1,jj-1) * pv(ji+1,jj-1,jk) - e2v(ji,jj-1) * pv(ji,jj-1,jk)  &
               &         - e1u(ji  ,jj  ) * pu(ji  ,jj  ,jk) + e1u(ji,jj-1) * pu(ji,jj-1,jk) )
            zcur_im1 = ahmf(ji-1,jj  ,jk) * e3f_0(ji-1,jj  ,jk) * r1_e1e2f(ji-1,jj  )         &   ! ahmf already * by fmask
            ...
            zdiv     = ahmt(ji,jj,jk) * r1_e1e2t(ji,jj) / e3t_0(ji,jj,jk)               &   ! ahmt already * by tmask
            ...
            zdiv_ip1 = ahmt(ji+1,jj,jk) * r1_e1e2t(ji+1,jj) / e3t_0(ji+1,jj,jk)         &   ! ahmt already * by tmask
               &     * ( e2u(ji+1,jj)*e3u_0(ji+1,jj,jk) * pu(ji+1,jj,jk) - e2u(ji,jj)*e3u_0(ji,jj,jk) * pu(ji,jj,jk)  &
               &       + e1v(ji+1,jj)*e3v_0(ji+1,jj,jk) * pv(ji+1,jj,jk) - e1v(ji+1,jj-1)*e3v_0(ji+1,jj-1,jk) * pv(ji+1,jj-1,jk) )
            zdiv_jp1 = ahmt(ji,jj+1,jk) * r1_e1e2t(ji,jj+1) / e3t_0(ji,jj+1,jk)         &   ! ahmt already * by tmask
               &     * ( e2u(ji,jj+1)*e3u_0(ji,jj+1,jk) * pu(ji,jj+1,jk) - e2u(ji-1,jj+1)*e3u_0(ji-1,jj+1,jk) * pu(ji-1,jj+1,jk)  &
               &       + e1v(ji,jj+1)*e3v_0(ji,jj+1,jk) * pv(ji,jj+1,jk) - e1v(ji,jj)*e3v_0(ji,jj,jk) * pv(ji,jj,jk) )
            !                                      ! - curl( curl) + grad( div )
            pu_rhs(ji,jj,jk) = pu_rhs(ji,jj,jk) + zsign * umask(ji,jj,jk) * (    &    ! * by umask is mandatory for dyn_ldf_blp use
               &              - ( zcur - zcur_jm1 ) * r1_e2u(ji,jj) / e3u_0(ji,jj,jk)   &
               &              + ( zdiv_ip1 - zdiv ) * r1_e1u(ji,jj)                      )
            !
            pv_rhs(ji,jj,jk) = pv_rhs(ji,jj,jk) + zsign * vmask(ji,jj,jk) * (    &    ! * by vmask is mandatory for dyn_ldf_blp use
               &                ( zcur - zcur_im1 ) * r1_e1v(ji,jj) / e3v_0(ji,jj,jk)   &
      ...
      CASE ( np_typ_sym )       !==  Symmetric operator  ==!
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)                           ! Horizontal slab
            ...
            pu_rhs(ji,jj,jk) = pu_rhs(ji,jj,jk) + zsign * r1_e1e2u(ji,jj) / e3u_0(ji,jj,jk)                    &
               &    * (   (   zten_ip1 * e2t(ji+1,jj  )*e2t(ji+1,jj  ) * e3t_0(ji+1,jj  ,jk)                   &
               &            - zten * e2t(ji  ,jj  )*e2t(ji  ,jj  ) * e3t_0(ji  ,jj  ,jk) ) * r1_e2u(ji,jj)     & 
               &        + (   zshe * e1f(ji  ,jj  )*e1f(ji  ,jj  ) * e3f_0(ji  ,jj  ,jk)                           &
               &            - zshe_jm1 * e1f(ji  ,jj-1)*e1f(ji  ,jj-1) * e3f_0(ji  ,jj-1,jk)     ) * r1_e1u(ji,jj) )   
            !
            pv_rhs(ji,jj,jk) = pv_rhs(ji,jj,jk) + zsign * r1_e1e2v(ji,jj) / e3v_0(ji,jj,jk)                    &
               &    * (   (   zshe * e2f(ji  ,jj  )*e2f(ji  ,jj  ) * e3f_0(ji  ,jj  ,jk)                           &
               &            - zshe_im1 * e2f(ji-1,jj  )*e2f(ji-1,jj  ) * e3f_0(ji-1,jj  ,jk)     ) * r1_e2v(ji,jj) &
               &        - (   zten_jp1 * e1t(ji  ,jj+1)*e1t(ji  ,jj+1) * e3t_0(ji  ,jj+1,jk)                   &
               &            - zten * e1t(ji  ,jj  )*e1t(ji  ,jj  ) * e3t_0(ji  ,jj  ,jk) ) * r1_e1v(ji,jj) )
            !
