      ...
      IF( kpass == 1 ) THEN         !==  first pass only  and whatever the tracer is  ==!
         ...
         DO kp = 0, 1                            ! i-k triads
            DO jk =  1,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
               ze3wr = 1._wp / e3w_0(ji,jj,jk+kp)
               zbu   = e1e2u(ji,jj) * e3u_0(ji,jj,jk)
               zbu1  = e1e2u(ji-1,jj) * e3u_0(ji-1,jj,jk)
               ...
               zslope2 = triadi_g(ji,jj,jk,1,kp) + ( gdept_0(ji+1,jj,jk) - gdept_0(ji,jj,jk) ) * r1_e1u(ji,jj) * umask(ji,jj,jk+kp)
               zslope2 = zslope2 *zslope2
               zslope21 = triadi_g(ji,jj,jk,0,kp) + ( gdept_0(ji,jj,jk) - gdept_0(ji-1,jj,jk) ) * r1_e1u(ji-1,jj) * umask(ji-1,jj,jk+kp)
         ...
         DO kp = 0, 1                            ! j-k triads
            DO jk =  1,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
               ze3wr = 1.0_wp / e3w_0(ji,jj,jk+kp)
               zbv   = e1e2v(ji,jj) * e3v_0(ji,jj,jk)
               zbv1   = e1e2v(ji,jj-1) * e3v_0(ji,jj-1,jk)
               ...
               zslope2 = triadj_g(ji,jj,jk,1,kp) + ( gdept_0(ji,jj+1,jk) - gdept_0(ji,jj,jk) ) * r1_e2v(ji,jj) * vmask(ji,jj,jk+kp)
               zslope2 = zslope2 * zslope2
               zslope21 = triadj_g(ji,jj,jk,0,kp) + ( gdept_0(ji,jj,jk) - gdept_0(ji,jj-1,jk) ) * r1_e2v(ji,jj-1) * vmask(ji,jj-1,jk+kp)
               zslope21 = zslope21 * zslope21
               ! round brackets added to fix the order of floating point operations
               ! needed to ensure halo 1 - halo 2 compatibility
            ...
            IF( ln_traldf_blp ) THEN                ! bilaplacian operator
               DO jk =  2,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
                  akz(ji,jj,jk) = 16._wp           &
                     ...
                     &   * (  akz     (ji,jj,jk)   &
                     &      + ah_wslp2(ji,jj,jk)   &
                     &        / ( e3w_0(ji,jj,jk) * e3w_0(ji,jj,jk) )  )
               END DO   ;   END DO   ;   END DO
            ELSEIF( ln_traldf_lap ) THEN              ! laplacian operator
               DO jk =  2,  jpkm1  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
                  ze3w_2 = e3w_0(ji,jj,jk) * e3w_0(ji,jj,jk)
...
!!gm  this should probably be done outside the jn loop
            ...
            IF( ln_botmix_triad ) THEN
               DO kp = 0, 1              !==  Horizontal & vertical fluxes
                  DO jj = ntsj-( iij), ntej+( iij-1 ) ; DO ji = ntsi-( iij), ntei+( iij-1)
                     ...
                     ze3wr = 1._wp / e3w_0(ji,jj,jk+kp)
                     ze3wr_ip1 = 1._wp / e3w_0(ji+1,jj,jk+kp)
                     zdzt  = zdkt3d(ji,jj,kp) * ze3wr
                     zdzt_ip1  = zdkt3d(ji+1,jj,kp) * ze3wr_ip1
                     !
                     zbu = 0.25_wp * e1e2u(ji,jj) * e3u_0(ji,jj,jk)
                     zbu_ip1 = 0.25_wp * e1e2u(ji+1,jj) * e3u_0(ji+1,jj,jk)
               ...
               DO kp = 0, 1
                  DO jj = ntsj-( iij), ntej+( iij-1 ) ; DO ji = ntsi-( iij), ntei+( iij-1)
                     ...
                     ze3wr = 1._wp / e3w_0(ji,jj,jk+kp)
                     ze3wr_jp1 = 1._wp / e3w_0(ji,jj+1,jk+kp)
                     zdzt  = zdkt3d(ji,jj,kp) * ze3wr
                     zdzt_jp1  = zdkt3d(ji,jj+1,kp) * ze3wr_jp1
                     zbv = 0.25_wp * e1e2v(ji,jj) * e3v_0(ji,jj,jk)
                     zbv_jp1 = 0.25_wp * e1e2v(ji,jj+1) * e3v_0(ji,jj+1,jk)
                     ! ln_botmix_triad is .T. don't mask zah for bottom half cells    !!gm ?????   ahu is masked....
         ...
         IF( ln_traldf_lap ) THEN               ! laplacian case: eddy coef = ah_wslp2 - akz
            DO jk =  2,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)
               ztfw(ji,jj,jk) = ztfw(ji,jj,jk) - e1e2t(ji,jj) / e3w_0(ji,jj,jk) * tmask(ji,jj,jk)   &
