   ...
   SUBROUTINE p4z_bc( kt, Kbb, Kmm, Krhs )
      ...
      IF( ll_dust )  THEN
         ...
         IF ( ln_trc_sbc(jpfer) ) THEN
            DO jk =  2,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
               zdustdep = dust(ji,jj) * zwdust * rfact * EXP( -gdept_0(ji,jj,jk) /( 250. * wdust ) )
               !
      ...
      IF( ll_ndepo ) THEN
         IF( ln_trc_sbc(jpno3) ) THEN
            ...
            DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
               zndep = rf_trsfac(jl) * sf_trcsbc(jl)%fnow(ji,jj,1) / e3t_0(ji,jj,1) / rn_sbc_time
      ...
      IF( ln_hydrofe ) THEN
         ...
         DO jk = 1, jpk
            hydrofe(:,:,jk) = ( MAX( rtrn, sf_hydrofe(1)%fnow(:,:,jk) ) * hratio ) &
              &              / ( e1e2t(:,:) * e3t_0(:,:,jk) * ryyss + rtrn ) / 1000._wp &
              &              * tmask(:,:,jk)
         ENDDO
