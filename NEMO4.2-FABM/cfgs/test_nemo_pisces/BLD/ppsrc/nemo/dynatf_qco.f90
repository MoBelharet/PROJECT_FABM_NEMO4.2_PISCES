   ...
   SUBROUTINE dyn_atf_qco( kt, Kbb, Kmm, Kaa, puu, pvv )
      ...
      IF( .NOT. l_1st_euler ) THEN    !* Leap-Frog : Asselin time filter
         ...
         IF( ln_dynspg_ts .AND. ln_bt_fw ) THEN
            ...
            ! END DO
            zue(:,:) = e3u_0(:,:,1) * puu(:,:,1,Kmm) * umask(:,:,1)
            zve(:,:) = e3v_0(:,:,1) * pvv(:,:,1,Kmm) * vmask(:,:,1)
            DO jk = 2, jpkm1
               zue(:,:) = zue(:,:) + e3u_0(:,:,jk) * puu(:,:,jk,Kmm) * umask(:,:,jk)
               zve(:,:) = zve(:,:) + e3v_0(:,:,jk) * pvv(:,:,jk,Kmm) * vmask(:,:,jk)
            END DO
            DO jk = 1, jpkm1
               puu(:,:,jk,Kmm) = puu(:,:,jk,Kmm) - (zue(:,:) * r1_hu_0(:,:) - uu_b(:,:,Kmm)) * umask(:,:,jk)
               pvv(:,:,jk,Kmm) = pvv(:,:,jk,Kmm) - (zve(:,:) * r1_hv_0(:,:) - vv_b(:,:,Kmm)) * vmask(:,:,jk)
            END DO
         ENDIF
         !
      ...
      uu_b(:,:,Kaa) = e3u_0(:,:,1) * puu(:,:,1,Kaa) * umask(:,:,1)
      uu_b(:,:,Kmm) = e3u_0(:,:,1) * puu(:,:,1,Kmm) * umask(:,:,1)
      vv_b(:,:,Kaa) = e3v_0(:,:,1) * pvv(:,:,1,Kaa) * vmask(:,:,1)
      vv_b(:,:,Kmm) = e3v_0(:,:,1) * pvv(:,:,1,Kmm) * vmask(:,:,1)
      DO jk = 2, jpkm1
         uu_b(:,:,Kaa) = uu_b(:,:,Kaa) + e3u_0(:,:,jk) * puu(:,:,jk,Kaa) * umask(:,:,jk)
         uu_b(:,:,Kmm) = uu_b(:,:,Kmm) + e3u_0(:,:,jk) * puu(:,:,jk,Kmm) * umask(:,:,jk)
         vv_b(:,:,Kaa) = vv_b(:,:,Kaa) + e3v_0(:,:,jk) * pvv(:,:,jk,Kaa) * vmask(:,:,jk)
         vv_b(:,:,Kmm) = vv_b(:,:,Kmm) + e3v_0(:,:,jk) * pvv(:,:,jk,Kmm) * vmask(:,:,jk)
      END DO
      uu_b(:,:,Kaa) = uu_b(:,:,Kaa) * r1_hu_0(:,:)
      vv_b(:,:,Kaa) = vv_b(:,:,Kaa) * r1_hv_0(:,:)
