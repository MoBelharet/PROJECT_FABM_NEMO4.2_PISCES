...
CONTAINS
   ...
   SUBROUTINE p5z_meso( kt, knt, Kbb, Kmm, Krhs )
      ...
      DO jk =  1,  jpkm1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         ...
         ! ---------------------------------------------------------------------  
         zproport  = 0._wp
         IF( gdepw_0(ji,jj,jk+1) > MAX(hmld(ji,jj), heup_01(ji,jj) ) ) THEN
      ...
      IF( ln_dvm_meso ) THEN
          ...
          DO jk =  1,  jpkm1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
             ...
             IF( gdept_0(ji,jj,jk) <= heup(ji,jj) ) THEN
                zmigthick  = e3t_0(ji,jj,jk) * tmask(ji,jj,jk) * ( 1. - zmigreltime ) 
   ...
   SUBROUTINE p5z_meso_depmig( Kbb, Kmm )
      ...
      DO jk =  1,  jpk  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         IF( tmask(ji,jj,jk) == 1.) THEN
            IF( gdept_0(ji,jj,jk) >= 150. .AND. gdept_0(ji,jj,jk) <= 500.) THEN
               oxymoy(ji,jj)  = oxymoy(ji,jj)  + tr(ji,jj,jk,jpoxy,Kbb) * 1E6 * e3t_0(ji,jj,jk)
               tempmoy(ji,jj) = tempmoy(ji,jj) + ts(ji,jj,jk,jp_tem,kmm)      * e3t_0(ji,jj,jk)
      ...
      ! ------------------------------------------
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         IF( depmig(ji,jj) >= gdepw_0(ji,jj,jk) .AND. depmig(ji,jj) < gdepw_0(ji,jj,jk+1) ) THEN
             kmig(ji,jj) = jk
          ENDIF
      END DO   ;   END DO   ;   END DO
      !
