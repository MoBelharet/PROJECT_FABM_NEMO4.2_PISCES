...
!!gm please remplace the '???' by true comments
      ...
      DO jk =  2,  jpk  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         ...
         zparr(ji,jj,jk) = zparr(ji,jj,jk-1) * EXP( -zkr * e3t_0(ji,jj,jk-1) )
         zparg(ji,jj,jk) = zparg(ji,jj,jk-1) * EXP( -zkg * e3t_0(ji,jj,jk-1) )
      ...
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls) ! mean par at t-levels
         ...
         zkr  = xkr0 + xkrp * EXP( xlr * zpig )
         zkg  = xkg0 + xkgp * EXP( xlg * zpig )
         zparr(ji,jj,jk) = zparr(ji,jj,jk) / ( zkr * e3t_0(ji,jj,jk) ) * ( 1 - EXP( -zkr * e3t_0(ji,jj,jk) ) )
         zparg(ji,jj,jk) = zparg(ji,jj,jk) / ( zkg * e3t_0(ji,jj,jk) ) * ( 1 - EXP( -zkg * e3t_0(ji,jj,jk) ) )
      ...
      DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls) 
         heup(ji,jj) = gdepw_0(ji,jj,neln(ji,jj))
      END DO   ;   END DO
