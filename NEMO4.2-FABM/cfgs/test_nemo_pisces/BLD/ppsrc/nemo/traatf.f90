...
CONTAINS
   ...
   SUBROUTINE tra_atf( kt, Kbb, Kmm, Kaa, pts )
      ...
      IF( l_trdtra )   THEN
         ...
         DO jk = 1, jpkm1
            ztrdt(:,:,jk) = ( pts(:,:,jk,jp_tem,Kaa)*e3t_0(:,:,jk) / e3t_0(:,:,jk) - pts(:,:,jk,jp_tem,Kmm)) * zfact
            ztrds(:,:,jk) = ( pts(:,:,jk,jp_sal,Kaa)*e3t_0(:,:,jk) / e3t_0(:,:,jk) - pts(:,:,jk,jp_sal,Kmm)) * zfact
         END DO
   ...
   SUBROUTINE tra_atf_vvl( kt, Kbb, Kmm, Kaa, kit000, p2dt, cdtype, pt, psbc_tc, psbc_tc_b, kjpt )
      ...
      DO jn = 1, kjpt
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
            ...
            zscale = tmask(ji,jj,jk) * e3t_0(ji,jj,jk) / ( ht_0(ji,jj) + 1._wp - ssmask(ji,jj) )
               ...
               IF ( ln_isfpar_mlt ) THEN
                  ...
                  IF ( jk >= misfkt_par(ji,jj) .AND. jk < misfkb_par(ji,jj) ) THEN
                     ztc_f  = ztc_f  - zfact1 * ( risf_par_tsc(ji,jj,jn) - risf_par_tsc_b(ji,jj,jn) )  &
                            &                 * e3t_0(ji,jj,jk) / rhisf_tbl_par(ji,jj)
                  END IF
                  ! level partially include in Losch_2008 ice shelf boundary layer
                  IF ( jk == misfkb_par(ji,jj) ) THEN
                     ztc_f  = ztc_f  - zfact1 * ( risf_par_tsc(ji,jj,jn) - risf_par_tsc_b(ji,jj,jn) )  &
                            &                 * e3t_0(ji,jj,jk) / rhisf_tbl_par(ji,jj) * rfrac_tbl_par(ji,jj)
                  END IF
               END IF
               !
