   ...
   SUBROUTINE flo_add_new_floats(Kmm, kfl_start, kfl_end)
         ...
         DO ji = MAX(Nis0,2), Nie0
            DO jj = MAX(Njs0,2), Nje0   ! NO vector opt.
               ...
               IF( llinmesh )THEN
                  ...
                  DO jk = 1, jpk-1
                     IF( (gdepw_0(ji,jj,jk) <= flzz(jfl)) .AND. (gdepw_0(ji,jj,jk+1) > flzz(jfl)) ) THEN
                        ikmfl(jfl) = jk
      ...
      DO jfl = kfl_start,kfl_end
         ...
         IF( (iimfl(jfl) >= 0) .AND. (ijmfl(jfl) >= 0) ) THEN
            ...
            zgkfl(jfl) = (( gdepw_0(iimfl(jfl),ijmfl(jfl),ikmfl(jfl)+1) - flzz(jfl) )* ikmfl(jfl))   &
               &                 / (  gdepw_0(iimfl(jfl),ijmfl(jfl),ikmfl(jfl)+1)                              &
               &                    - gdepw_0(iimfl(jfl),ijmfl(jfl),ikmfl(jfl) ) )                             &
               &                 + (( flzz(jfl)-gdepw_0(iimfl(jfl),ijmfl(jfl),ikmfl(jfl)) ) *(ikmfl(jfl)+1))   &
               &                 / (  gdepw_0(iimfl(jfl),ijmfl(jfl),ikmfl(jfl)+1)                              &
               &                    - gdepw_0(iimfl(jfl),ijmfl(jfl),ikmfl(jfl)) )
         ELSE
            zgifl(jfl) = 0.e0
