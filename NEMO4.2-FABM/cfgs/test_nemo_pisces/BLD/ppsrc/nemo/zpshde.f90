...
CONTAINS
   ...
   SUBROUTINE zps_hde_t( kt, kjpt, pta, ktta, pgtu, pgtv, ktgt,   &
      ...
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktta+1:,(ntsj-nn_hls-1)*ktta+1:,:,:), INTENT(in   )           ::  pta         ! 4D tracers fields
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgt+1:,(ntsj-nn_hls-1)*ktgt+1:    ,:), INTENT(  out)           ::  pgtu, pgtv  ! hor. grad. of ptra at u- & v-pts
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktrd+1:,(ntsj-nn_hls-1)*ktrd+1:,:     ), INTENT(in   ), OPTIONAL ::  prd         ! 3D density anomaly fields
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgr+1:,(ntsj-nn_hls-1)*ktgr+1:         ), INTENT(  out), OPTIONAL ::  pgru, pgrv  ! hor. grad of prd at u- & v-pts (bottom)
      !
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))      ::   zri, zrj, zhi, zhj   ! NB: 3rd dim=1 to use eos
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),kjpt) ::   zti, ztj             !
      !!----------------------------------------------------------------------
      !
      ...
      DO jn = 1, kjpt      !==   Interpolation of tracers at the last ocean level   ==!
         !
         DO jj = ntsj-( nn_hls), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls-1)              ! Gradient of density at the last level
      ...
      IF( PRESENT( prd ) ) THEN    !==  horizontal derivative of density anomalies (rd)  ==!    (optional part)
         ...
         !
         DO jj = ntsj-( nn_hls-1), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls-1)              ! Gradient of density at the last level
            iku = mbku(ji,jj)
            ikv = mbkv(ji,jj)
            ze3wu  = e3w_0(ji+1,jj  ,iku) - e3w_0(ji,jj,iku)
            ze3wv  = e3w_0(ji  ,jj+1,ikv) - e3w_0(ji,jj,ikv)
   ...
   SUBROUTINE zps_hde_isf_t( kt, kjpt, pta, ktta, pgtu, pgtv, ktgt, pgtui, pgtvi, ktgti,  &
      ...
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktta+1:,(ntsj-nn_hls-1)*ktta+1:,:,:), INTENT(in   )           ::  pta          ! 4D tracers fields
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgt+1:,(ntsj-nn_hls-1)*ktgt+1:    ,:), INTENT(  out)           ::  pgtu, pgtv   ! hor. grad. of ptra at u- & v-pts
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgti+1:,(ntsj-nn_hls-1)*ktgti+1:   ,:), INTENT(  out)           ::  pgtui, pgtvi ! hor. grad. of stra at u- & v-pts (ISF)
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktrd+1:,(ntsj-nn_hls-1)*ktrd+1:,:     ), INTENT(in   ), OPTIONAL ::  prd          ! 3D density anomaly fields
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgr+1:,(ntsj-nn_hls-1)*ktgr+1:         ), INTENT(  out), OPTIONAL ::  pgru, pgrv   ! hor. grad of prd at u- & v-pts (bottom)
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgri+1:,(ntsj-nn_hls-1)*ktgri+1:        ), INTENT(  out), OPTIONAL ::  pgrui, pgrvi ! hor. grad of prd at u- & v-pts (top)
      !
      INTEGER  ::   ji, jj, jn      ! Dummy loop indices
