   ...
   SUBROUTINE p4z_poc( kt, knt, Kbb, Kmm, Krhs )
     ...
     DO jk =  2,  jpkm1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
        IF (tmask(ji,jj,jk) == 1.) THEN
          ...
          ! ------------------------------------------------------------
          !
          IF( gdept_0(ji,jj,jk) > zdep ) THEN
            alphat = 0.
            remint = 0.
            !
            zsizek1  = e3t_0(ji,jj,jk-1) / 2. / (wsbio4(ji,jj,jk-1) + rtrn) * tgfunc(ji,jj,jk-1)
            zsizek = e3t_0(ji,jj,jk) / 2. / (wsbio4(ji,jj,jk) + rtrn) * tgfunc(ji,jj,jk)
            !
            IF ( gdept_0(ji,jj,jk-1) <= zdep ) THEN
              ! 
              ...
              &   * e3t_0(ji,jj,jk) / 2. / (wsbio4(ji,jj,jk) + rtrn)
              zpoc = MAX(0., zpoc)
              !
              DO jn = 1, jcpoc
                 !
            ...
            ELSE
              ...
              &   * e3t_0(ji,jj,jk-1) / 2. / (wsbio4(ji,jj,jk-1) + rtrn) + consgoc(ji,jj,jk)   &
              &   * rday / rfact2 * e3t_0(ji,jj,jk) / 2. / (wsbio4(ji,jj,jk) + rtrn)
              zpoc = max(0., zpoc)
              !
              DO jn = 1, jcpoc
     ...
     DO jk =  1,  jpkm1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
        zdep = hmld(ji,jj)
        IF (tmask(ji,jj,jk) == 1. .AND. gdept_0(ji,jj,jk) <= zdep ) THEN
          totprod(ji,jj) = totprod(ji,jj) + prodpoc(ji,jj,jk) * e3t_0(ji,jj,jk) * rday/ rfact2
          ! The temperature effect is included here
          totthick(ji,jj) = totthick(ji,jj) + e3t_0(ji,jj,jk)* tgfunc(ji,jj,jk)
          totcons(ji,jj) = totcons(ji,jj) - conspoc(ji,jj,jk) * e3t_0(ji,jj,jk) * rday/ rfact2    &
     ...
     DO jk =  2,  jpkm1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
        IF (tmask(ji,jj,jk) == 1.) THEN
          ...
          IF( gdept_0(ji,jj,jk) > zdep ) THEN
            ...
            zsizek1  = e3t_0(ji,jj,jk-1) / 2. / (wsbio3(ji,jj,jk-1) + rtrn) * tgfunc(ji,jj,jk-1)
            zsizek = e3t_0(ji,jj,jk) / 2. / (wsbio3(ji,jj,jk) + rtrn) * tgfunc(ji,jj,jk)
            !
            ...
            IF ( gdept_0(ji,jj,jk-1) <= zdep ) THEN
              ...
              &   * e3t_0(ji,jj,jk) / 2. / (wsbio3(ji,jj,jk) + rtrn)
              zpoc = max(0., zpoc)
              ! 
              DO jn = 1, jcpoc
            ...
            ELSE
              ...
              &   * e3t_0(ji,jj,jk-1) / 2. / (wsbio3(ji,jj,jk-1) + rtrn) + conspoc(ji,jj,jk)   &
              &   * rday / rfact2 * e3t_0(ji,jj,jk) / 2. / (wsbio3(ji,jj,jk) + rtrn)
              zpoc = max(0., zpoc)
              !
              DO jn = 1, jcpoc
