   ...
   SUBROUTINE sbc_stokes( Kmm )
      ...
      !# define zInt_w ze3divh
         DO jk =  1,  jpk  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1) ! Compute the primitive of Breivik 2016 function at W-points
            zfac             = - 2._wp * zk_t (ji,jj) * gdepw_0(ji,jj,jk)  !<-- zfac should be negative definite
            ztemp            = EXP ( zfac )
            zsqrt            = SQRT( -zfac )
            zbreiv16_w       = ztemp - SQRT(rpi)*zsqrt*ERFC(zsqrt) !Eq. 16 Breivik 2016
            zInt_w(ji,jj,jk) = ztemp - 4._wp * zk_t (ji,jj) * gdepw_0(ji,jj,jk) * zbreiv16_w
...
!
      ...
      !                       !==  horizontal Stokes Drift 3D velocity  ==!
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
            zdep_u = 0.5_wp * ( gdept_0(ji,jj,jk) + gdept_0(ji+1,jj,jk) )
            zdep_v = 0.5_wp * ( gdept_0(ji,jj,jk) + gdept_0(ji,jj+1,jk) )
            !
      ...
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  1) ; DO ji = ntsi-( 0), ntei+(  1)    ! Horizontal e3*divergence
         ze3divh(ji,jj,jk) = (  e2u(ji  ,jj) * e3u_0(ji  ,jj,jk) * usd(ji  ,jj,jk)    &
            &                 - e2u(ji-1,jj) * e3u_0(ji-1,jj,jk) * usd(ji-1,jj,jk)    &
            &                 + e1v(ji,jj  ) * e3v_0(ji,jj  ,jk) * vsd(ji,jj  ,jk)    &
            &                 - e1v(ji,jj-1) * e3v_0(ji,jj-1,jk) * vsd(ji,jj-1,jk)  ) &
            &                * r1_e1e2t(ji,jj)
      END DO   ;   END DO   ;   END DO
      !
