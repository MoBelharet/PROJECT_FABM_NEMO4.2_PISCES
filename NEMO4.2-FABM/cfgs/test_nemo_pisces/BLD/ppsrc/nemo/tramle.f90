   ...
   SUBROUTINE tra_mle_trp( kt, kit000, pu, pv, pw, cdtype, Kmm )
      ...
      ELSE !do not use osn_mle
         ...
         DO jk =  1,  ikmax  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)                    ! MLD and mean buoyancy and N2 over the mixed layer
            zc = e3t_0(ji,jj,jk) * REAL( MIN( MAX( 0, inml_mle(ji,jj)-jk ) , 1  )  )    ! zc being 0 outside the ML t-points
         ...
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
            zbm(ji,jj) = + grav * zbm(ji,jj) / MAX( e3t_0(ji,jj,1), zmld(ji,jj) )
         END DO   ;   END DO
         !
         !
    ...
    !
      DO jk =  2,  ikmax  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls-1)                ! start from 2 : surface value = 0
         ...
         zcuw = 1._wp - ( gdepw_0(ji+1,jj,jk) + gdepw_0(ji,jj,jk) ) * zhu(ji,jj)
         zcvw = 1._wp - ( gdepw_0(ji,jj+1,jk) + gdepw_0(ji,jj,jk) ) * zhv(ji,jj)
         zcuw = zcuw * zcuw
         zcvw = zcvw * zcvw
