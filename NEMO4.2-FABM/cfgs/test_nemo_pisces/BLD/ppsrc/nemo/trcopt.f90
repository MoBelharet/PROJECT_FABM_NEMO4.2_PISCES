...
CONTAINS
   ...
   SUBROUTINE trc_opt( kt, knt, Kbb, Kmm, zchl, ze1, ze2, ze3)
      ...
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         ...
         !                                                         
         ekb(ji,jj,jk) = xkrgb(1,irgb) * e3t_0(ji,jj,jk)
         ekg(ji,jj,jk) = xkrgb(2,irgb) * e3t_0(ji,jj,jk)
         ekr(ji,jj,jk) = xkrgb(3,irgb) * e3t_0(ji,jj,jk)
      ...
      IF( ln_qsr_bio ) THEN
         ...
         DO jk = 2, nksrp + 1
            DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
                  ze0(ji,jj,jk) = ze0(ji,jj,jk-1) * EXP( -e3t_0(ji,jj,jk-1) * (1. / rn_si0) )
      ...
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         ...
         zeps(ji,jj,jk) = ztmp / e3t_0(ji,jj,jk) / (etot(ji,jj,jk) + rtrn)
      END DO   ;   END DO   ;   END DO
      !zeps = (ze1(:,:,:) * ekb(:,:,:) + ze2(:,:,:) * ekg(:,:,:) + ze3(:,:,:) * ekr(:,:,:)) / e3t_0(:,:,:) / (etot(:,:,:) + rtrn)
