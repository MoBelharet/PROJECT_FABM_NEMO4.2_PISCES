...
CONTAINS
   ...
   SUBROUTINE ldf_slp( kt, prd, pn2, Kbb, Kmm )
      ...
      DO jk =  2,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)        !* Slopes at u and v points
         ...
         zbu = MIN(  zbu, - z1_slpmax * ABS( zau ) , -7.e+3_wp/e3u_0(ji,jj,jk)* ABS( zau )  )
         zbv = MIN(  zbv, - z1_slpmax * ABS( zav ) , -7.e+3_wp/e3v_0(ji,jj,jk)* ABS( zav )  )
         !                                      ! Fred Dupont: add a correction for bottom partial steps:
         !                                      !              max slope = 1/2 * e3 / e1
         IF (ln_zps .AND. jk==mbku(ji,jj)) &
            zbu = MIN(  zbu, - z1_slpmax * ABS( zau ) ,   &
               &                - 2._wp * e1u(ji,jj) / e3u_0(ji,jj,jk)* ABS( zau )  )
         IF (ln_zps .AND. jk==mbkv(ji,jj)) &
            zbv = MIN(  zbv, - z1_slpmax * ABS( zav ) ,   &
               &                - 2._wp * e2v(ji,jj) / e3v_0(ji,jj,jk)* ABS( zav )  )
...
!!gm end modif
      ...
      DO jk =  2,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
         ...
         zck = ( gdepw_0(ji,jj,jk) - gdepw_0(ji,jj,mikt(ji,jj)) ) / MAX( hmlp(ji,jj) - gdepw_0(ji,jj,mikt(ji,jj)), 10._wp )
...
!!gm end modif
            ...
            DO jj = ntsj-( nn_hls), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls-1)
               ...
               ELSE                             
                  ...
                  zti_g_raw = (  zdxrho(ji+ip,jj,jk-kp,1-ip) / zdzrho(ji+ip,jj,jk-kp,kp)      &
                     &          - ( gdept_0(ji+1,jj,jk-kp) - gdept_0(ji,jj,jk-kp) ) * r1_e1u(ji,jj)  ) * umask(ji,jj,jk)
                  ze3_e1    =  e3w_0(ji+ip,jj,jk-kp) * r1_e1u(ji,jj) 
               ...
               ELSE
                  ...
                  ze3_e2    =  e3w_0(ji,jj+jp,jk-kp) / e2v(ji,jj)
         ...
         DO jl = 0, 1
            ...
            DO jk = 1, jpkm1
               ...
               DO jj = ntsj-( nn_hls), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls-1)
                  ...
                  zti_coord = znot_thru_surface * ( gdept_0(ji+1,jj  ,jk) - gdept_0(ji,jj,jk) ) * r1_e1u(ji,jj)
                  ztj_coord = znot_thru_surface * ( gdept_0(ji  ,jj+1,jk) - gdept_0(ji,jj,jk) ) * r1_e2v(ji,jj)     ! unmasked
                  ...
                  ze3_e1    = e3w_0(ji+ip,jj   ,jk+kp) * r1_e1u(ji,jj)
                  ze3_e2    = e3w_0(ji   ,jj+jp,jk+kp) * r1_e2v(ji,jj)
                  ...
                  !
                  zbu  = e1e2u(ji   ,jj   ) * e3u_0(ji   ,jj   ,jk   )
                  zbv  = e1e2v(ji   ,jj   ) * e3v_0(ji   ,jj   ,jk   )
                  zbti = e1e2t(ji+ip,jj   ) * e3w_0(ji+ip,jj   ,jk+kp)
                  zbtj = e1e2t(ji   ,jj+jp) * e3w_0(ji   ,jj+jp,jk+kp)
                  !
   ...
   SUBROUTINE ldf_slp_mxl( prd, pn2, p_gru, p_grv, p_dzr, Kmm )
      ...
      DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
         ...
         !                        !- bound the slopes: abs(zw.)<= 1/100 and zb..<0
         !                           kxz max= ah slope max =< e1 e3 /(pi**2 2 dt)
         zbu = MIN(  zbu , - z1_slpmax * ABS( zau ) , -7.e+3_wp/e3u_0(ji,jj,iku)* ABS( zau )  )
         zbv = MIN(  zbv , - z1_slpmax * ABS( zav ) , -7.e+3_wp/e3v_0(ji,jj,ikv)* ABS( zav )  )
...
!                     wslpi(ji,jj,jk) = - ( gdepw_0(ji+1,jj,jk) - gdepw_0(ji-1,jj,jk) ) * r1_e1t(ji,jj) * wmask(ji,jj,jk) * 0.5
!                     wslpj(ji,jj,jk) = - ( gdepw_0(ji,jj+1,jk) - gdepw_0(ji,jj-1,jk) ) * r1_e2t(ji,jj) * wmask(ji,jj,jk) * 0.5
!                  END DO
!               END DO
!            END DO
