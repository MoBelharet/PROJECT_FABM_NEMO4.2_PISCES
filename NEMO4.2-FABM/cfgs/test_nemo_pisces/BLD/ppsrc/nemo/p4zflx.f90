...
CONTAINS
   ...
   SUBROUTINE p4z_flx ( kt, knt, Kbb, Kmm, Krhs )
      ...
      DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
         ...
         ! compute the trend
         tr(ji,jj,1,jpdic,Krhs) = tr(ji,jj,1,jpdic,Krhs) + oce_co2(ji,jj) * rfact2 / e3t_0(ji,jj,1)
         ...
         tr(ji,jj,1,jpoxy,Krhs) = tr(ji,jj,1,jpoxy,Krhs) + zoflx(ji,jj) * rfact2 / e3t_0(ji,jj,1)
      END DO   ;   END DO
