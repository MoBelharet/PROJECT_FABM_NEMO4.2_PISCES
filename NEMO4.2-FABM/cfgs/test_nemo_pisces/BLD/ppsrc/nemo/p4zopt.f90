...
CONTAINS
   ...
   SUBROUTINE p4z_opt( kt, knt, Kbb, Kmm )
      ...
      DO jk =  1,  jpkm1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         ...
         !                                                         
         ekb(ji,jj,jk) = rkrgb(1,irgb) * e3t_0(ji,jj,jk)
         ekg(ji,jj,jk) = rkrgb(2,irgb) * e3t_0(ji,jj,jk)
         ekr(ji,jj,jk) = rkrgb(3,irgb) * e3t_0(ji,jj,jk)
      ...
      DO jk =  2,  nksr ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
        IF( etot_ndcy(ji,jj,jk) * tmask(ji,jj,jk) >=  zqsr100(ji,jj) )  THEN
           ...
           heup(ji,jj) = gdepw_0(ji,jj,jk+1)     ! Euphotic layer depth
        ENDIF
        IF( etot_ndcy(ji,jj,jk) * tmask(ji,jj,jk) >= 0.10 )  THEN
           heup_01(ji,jj) = gdepw_0(ji,jj,jk+1)  ! Euphotic layer depth (light level definition)
        ENDIF
      END DO   ;   END DO   ;   END DO
      !
      ...
      DO jk =  1,  nksr ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         IF( gdepw_0(ji,jj,jk+1) <= hmld(ji,jj) ) THEN
            zetmp1 (ji,jj) = zetmp1 (ji,jj) + etot     (ji,jj,jk) * e3t_0(ji,jj,jk) ! Actual PAR for remineralisation
            zetmp2 (ji,jj) = zetmp2 (ji,jj) + etot_ndcy(ji,jj,jk) * e3t_0(ji,jj,jk) ! Par averaged over 24h for production
            zdepmoy(ji,jj) = zdepmoy(ji,jj) +                       e3t_0(ji,jj,jk)
         ENDIF
      END DO   ;   END DO   ;   END DO
      !
      ...
      !
      DO jk =  1,  nksr ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
         IF( gdepw_0(ji,jj,jk+1) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
            zetmp3 (ji,jj) = zetmp3 (ji,jj) + enano    (ji,jj,jk) * e3t_0(ji,jj,jk) ! Nanophytoplankton
            zetmp4 (ji,jj) = zetmp4 (ji,jj) + ediat    (ji,jj,jk) * e3t_0(ji,jj,jk) ! Diatoms
            zdepmoy(ji,jj) = zdepmoy(ji,jj) +                       e3t_0(ji,jj,jk)
         ENDIF
      END DO   ;   END DO   ;   END DO
      enanom(:,:,:) = enano(:,:,:)
      ...
      IF( ln_p5z ) THEN
         ...
         DO jk =  1,  nksr ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
            IF( gdepw_0(ji,jj,jk+1) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
               zetmp5(ji,jj)  = zetmp5 (ji,jj) + epico(ji,jj,jk) * e3t_0(ji,jj,jk)
            ENDIF
         END DO   ;   END DO   ;   END DO
         !
         epicom(:,:,:) = epico(:,:,:)
   ...
   SUBROUTINE p4z_opt_par( kt, Kmm, pqsr, pe1, pe2, pe3, pe0, pqsr100 ) 
      ...
      IF( PRESENT( pe0 ) ) THEN     !  W-level
         ...
         DO jk =  2,  nksr + 1 ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
            pe0(ji,jj,jk) = pe0(ji,jj,jk-1) * EXP( -e3t_0(ji,jj,jk-1) * xsi0r )
