...
CONTAINS
   ...
   SUBROUTINE dyn_keg( kt, kscheme, Kmm, puu, pvv, Krhs )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk)    ::   zhke
      ...
      CASE ( nkeg_HW )                          !--  Hollingsworth scheme  --!
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  nn_hls-1) ; DO ji = ntsi-( 0), ntei+(  nn_hls-1)
      ...
      END SELECT 
      !
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)       !==  grad( KE ) added to the general momentum trends  ==!
