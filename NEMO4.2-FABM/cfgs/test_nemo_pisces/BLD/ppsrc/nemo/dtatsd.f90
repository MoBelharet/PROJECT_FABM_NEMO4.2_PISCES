...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtsd in configuration namelist' )
   ...
   SUBROUTINE dta_tsd( kt, ptsd )
      ...
      REAL(dp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk,jpts), INTENT(  out) ::   ptsd   ! T & S data
      !
...
!!gm end
      ...
      IF( ln_sco ) THEN                   !==   s- or mixed s-zps-coordinate   ==!
         ...
         ENDIF
         !
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)                  ! vertical interpolation of T & S
