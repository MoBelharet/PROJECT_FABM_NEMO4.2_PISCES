...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppmax_real_sp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(sp)         , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
      INTEGER, OPTIONAL, INTENT(in   ) ::   kcom        ! optional communicator
      !
      INTEGER :: ipi, ii, ierr
      ...
      ALLOCATE(work(ipi))
      IF( ln_timing ) CALL tic_tac(.TRUE., ld_global = .TRUE.)
      CALL mpi_allreduce( ptab, work, ipi, mpi_real, mpi_max, ilocalcomm, ierror )
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
      DO ii = 1, ipi
         ptab = work(ii)
      ENDDO
   ...
   END SUBROUTINE mppmax_real_sp

!                          !==  IN: ptab is an array  ==!

   SUBROUTINE mppmax_a_real_sp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(sp)         , INTENT(inout) ::   ptab(:)   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppmax_a_real_sp
...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppmax_real_dp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(dp)         , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
      INTEGER, OPTIONAL, INTENT(in   ) ::   kcom        ! optional communicator
      !
      INTEGER :: ipi, ii, ierr
   ...
   END SUBROUTINE mppmax_real_dp

!                          !==  IN: ptab is an array  ==!

   SUBROUTINE mppmax_a_real_dp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(dp)         , INTENT(inout) ::   ptab(:)   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppmax_a_real_dp
...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppmin_real_sp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(sp)         , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
      ...
      ALLOCATE(work(ipi))
      IF( ln_timing ) CALL tic_tac(.TRUE., ld_global = .TRUE.)
      CALL mpi_allreduce( ptab, work, ipi, mpi_real, mpi_min, ilocalcomm, ierror )
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
      DO ii = 1, ipi
         ptab = work(ii)
      ENDDO
   ...
   END SUBROUTINE mppmin_real_sp

!                          !==  IN: ptab is an array  ==!

   SUBROUTINE mppmin_a_real_sp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(sp)         , INTENT(inout) ::   ptab(:)   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppmin_a_real_sp
...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppmin_real_dp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(dp)         , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppmin_real_dp

!                          !==  IN: ptab is an array  ==!

   SUBROUTINE mppmin_a_real_dp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(dp)         , INTENT(inout) ::   ptab(:)   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppmin_a_real_dp
...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppsum_real_sp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(sp)         , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
      ...
      ALLOCATE(work(ipi))
      IF( ln_timing ) CALL tic_tac(.TRUE., ld_global = .TRUE.)
      CALL mpi_allreduce( ptab, work, ipi, mpi_real, mpi_sum, ilocalcomm, ierror )
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
      DO ii = 1, ipi
         ptab = work(ii)
      ENDDO
   ...
   END SUBROUTINE mppsum_real_sp

!                          !==  IN: ptab is an array  ==!

   SUBROUTINE mppsum_a_real_sp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(sp)         , INTENT(inout) ::   ptab(:)   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppsum_a_real_sp
...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppsum_real_dp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(dp)         , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppsum_real_dp

!                          !==  IN: ptab is an array  ==!

   SUBROUTINE mppsum_a_real_dp( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      REAL(dp)         , INTENT(inout) ::   ptab(:)   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
   ...
   END SUBROUTINE mppsum_a_real_dp
...
!                          !==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mppsum_realdd( cdname, ptab, kdim, kcom )
      !!----------------------------------------------------------------------
      CHARACTER(len=*),                   INTENT(in   ) ::   cdname  ! name of the calling subroutine
      COMPLEX(dp)       , INTENT(inout) ::   ptab   ! array or pointer of arrays on which the boundary condition is applied
      INTEGER, OPTIONAL, INTENT(in   ) ::   kdim        ! optional pointer dimension
      INTEGER, OPTIONAL, INTENT(in   ) ::   kcom        ! optional communicator
      !
      INTEGER :: ipi, ii, ierr
...
!==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mpp_minloc2d_sp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:)                     ! local mask
      REAL(sp)  , INTENT(  out) ::   pmin    ! Global minimum of ptab
      ...
      REAL(sp) ::   zmin     ! local minimum
      REAL(sp), DIMENSION(2,1) ::   zain, zaout
      LOGICAL  ::   llhalo
      !!-----------------------------------------------------------------------
      !
      IF( narea == 1 .AND. numcom == -1 ) CALL mpp_report( cdname, ld_glb = .TRUE. )
      ...
      idim = SIZE(kindex)
      !
      IF ( ANY( ldmsk(:,:) ) ) THEN   ! there is at least 1 valid point...
         !
         ALLOCATE ( ilocs(idim) )
         !
         ilocs = MINLOC( ptab(:,:) , mask= ldmsk(:,:) )
      ...
      !
      zain(1,:) = zmin
      zain(2,:) = REAL(index0, sp)
      !
      IF( ln_timing ) CALL tic_tac(.TRUE., ld_global = .TRUE.)
      CALL MPI_ALLREDUCE( zain, zaout, 1, MPI_2REAL, MPI_MINLOC ,MPI_COMM_OCE, ierror)
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
      !
      pmin      = zaout(1,1)
   ...
   END SUBROUTINE mpp_minloc2d_sp


!==  IN: ptab is an array  ==!


   SUBROUTINE mpp_minloc3d_sp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:,:)                     ! local mask
   ...
   END SUBROUTINE mpp_minloc3d_sp
...
!==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mpp_maxloc2d_sp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:)                     ! local mask
      ...
      !
      IF( ln_timing ) CALL tic_tac(.TRUE., ld_global = .TRUE.)
      CALL MPI_ALLREDUCE( zain, zaout, 1, MPI_2REAL, MPI_MAXLOC ,MPI_COMM_OCE, ierror)
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
      !
      pmin      = zaout(1,1)
      index0    = NINT( zaout(2,1) )
   ...
   END SUBROUTINE mpp_maxloc2d_sp


!==  IN: ptab is an array  ==!


   SUBROUTINE mpp_maxloc3d_sp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:,:)                     ! local mask
   ...
   END SUBROUTINE mpp_maxloc3d_sp
...
!==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mpp_minloc2d_dp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:)                     ! local mask
      REAL(dp)  , INTENT(  out) ::   pmin    ! Global minimum of ptab
      INTEGER         , INTENT(  out) ::   kindex(2)                                ! index of minimum in global frame
      LOGICAL, OPTIONAL, INTENT(in   ) ::   ldhalo  ! If .false. (default) excludes halos in kindex 
      !
      INTEGER  ::   ierror, ii, idim
   ...
   END SUBROUTINE mpp_minloc2d_dp


!==  IN: ptab is an array  ==!


   SUBROUTINE mpp_minloc3d_dp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:,:)                     ! local mask
   ...
   END SUBROUTINE mpp_minloc3d_dp
...
!==  IN: ptab is an array  ==!
   ...
   SUBROUTINE mpp_maxloc2d_dp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:)                     ! local mask
   ...
   END SUBROUTINE mpp_maxloc2d_dp


!==  IN: ptab is an array  ==!


   SUBROUTINE mpp_maxloc3d_dp( cdname, ptab, ldmsk, pmin, kindex, ldhalo )
      !!----------------------------------------------------------------------
      CHARACTER(len=*), INTENT(in    ) ::   cdname  ! name of the calling subroutine
      REAL(dp)        , INTENT(in   ) ::   ptab(:,:,:)                            ! array on which loctrans operation is applied
      LOGICAL          , INTENT(in   ) ::   ldmsk(:,:,:)                     ! local mask
   ...
   END SUBROUTINE mpp_maxloc3d_dp
